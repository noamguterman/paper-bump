...
public class AnchorToolsEditor : EditorWindow
{
	...
	static bool UGUI_Layout_AutoAnchor = false;
	...
	private static void OnScene(SceneView sceneview)
	{
		if (sceneview.in2DMode) {
			...
			EditorGUILayout.PrefixLabel ("Auto Anchor");
			UGUI_Layout_AutoAnchor = EditorGUILayout.Toggle(UGUI_Layout_AutoAnchor);
			EditorGUILayout.EndHorizontal();
			sceneview.EndWindows ();

            if (Event.current.type == EventType.MouseUp && Event.current.button == 0 && UGUI_Layout_AutoAnchor)
            {
                UpdateAnchors();
            }
        }
