...
//                Debug.Log(Time.frameCount + " <color=#FFEC03>newCompletedSteps = " + newCompletedSteps + "</color> - completedLoops: " + s.completedLoops + " - updateMode: " + updateMode);
                ...
                if (updateMode == UpdateMode.Update) {
                    ...
                    while (cyclesDone < cycles) {
                        ...
                        if (ApplypublicCycle(s, from, to, updateMode, useInversePosition, prevPosIsInverse, true)) return true;
                        cyclesDone++;
...
//                    Debug.Log("     public Cycle Ended > expecteCompletedLoops/completedLoops: " + expectedCompletedLoops + "/" + s.completedLoops + " - expectedPosition/position: " + expectedPosition + "/" + s.position);
            ...
            if (newCompletedSteps > 0 && !s.isComplete) {
                ...
                if (s.loopType == LoopType.Restart && to > 0) ApplypublicCycle(s, s.duration, 0, UpdateMode.Goto, false, false, false);
            } else from = useInversePosition ? s.duration - prevPos : prevPos;
            return ApplypublicCycle(s, from, useInversePosition ? s.duration - newPos : newPos, updateMode, useInversePosition, prevPosIsInverse);
        }
        ...
        // Returns TRUE if the tween needs to be killed
        static bool ApplypublicCycle(Sequence s, float fromPos, float toPos, UpdateMode updateMode, bool useInverse, bool prevPosIsInverse, bool multiCycleStep = false)
        {
